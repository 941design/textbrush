...
export function parsePngMetadata(buffer) {
    try {
        ...
        if (generatedWidth !== undefined)
            result.generatedWidth = generatedWidth;
        const generatedHeight = getInt('GeneratedHeight');
        if (generatedHeight !== undefined)
            result.generatedHeight = generatedHeight;
        return result;
    }
    catch (error) {
 ...
 */
export async function fetchAndParsePngMetadata(url) {
    try {
