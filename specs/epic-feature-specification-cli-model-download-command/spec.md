---
epic: feature-specification-cli-model-download-command
created: 2026-02-21T18:00:36Z
status: initializing
---
...
**Error Handling:**
- `TokenRequiredError` (new exception in `textbrush/model/weights.py`): Print "HuggingFace token required. Set HF_TOKEN environment variable."
...
**Return Value:** `download_flux_weights()` must be updated to return the HuggingFace snapshot directory path (a `Path` object). Raise `TokenRequiredError` (defined in `textbrush/model/weights.py`) when no token is available or when auth fails (401/403).
...
**Behavior:**
- Check `HF_TOKEN` environment variable first (canonical env var used by HuggingFace hub library)
- Check `config.huggingface.token` from the already-loaded Config object if env var not set
- If neither available, `download_flux_weights()` raises `TokenRequiredError`; CLI handler prints:
  ```
...
**Changes Required:**
- Make `--prompt` optional (`required=False`) in `build_parser()`
- Add `--download-model` flag
- Update `validate_args()` to enforce: error if neither `--prompt` nor `--download-model` is provided; error if both `--download-model` and `--prompt` are provided; error if both `--download-model` and `--headless` are provided
- Handle download in `main()` before normal generation path
...
# --prompt: change required=True to required=False

# In validate_args():
if not args.prompt and not args.download_model:
...
if args.download_model:
    from textbrush.model.weights import download_flux_weights, TokenRequiredError
    try:
        ...
        print("Error: HuggingFace token required. Set HF_TOKEN environment variable.", file=sys.stderr)
        sys.exit(1)
